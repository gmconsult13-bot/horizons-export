...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const rooms = await adminClient.collection('rooms').getFullList({
      requestKey: null,
    });
    ...
    return res.json({ room_types: roomTypes });
  } catch (error) {
    logger.error(
      `Failed to fetch room allotments: ${
        error?.response?.message || error?.message || 'Unknown error'
      }`,
    );
    return res.status(Number(error?.status) || 500).json({
      error: 'Failed to load room allotments from the database',
    });
  }
});
...
router.get('/:roomTypeId/bookings', async (req, res) => {
  ...
  const adminClient = await createAuthenticatedSuperuserClient();
  const bookings = await adminClient.collection('bookings').getFullList({
