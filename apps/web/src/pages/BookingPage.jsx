...
const NON_REFUNDABLE_DISCOUNT_PERCENT = 10;
...
export default function BookingPage() {
      ...
      try {
        ...
        if (records.length > 0) {
          setFormData((previous) => ({
            ...previous,
            accommodationType: records[0].name,
          }));
        }
      } catch (error) {
  ...
  const selectedRoom = rooms.find(
    (room) => room.name === formData.accommodationType,
  );
  ...
  const handleChildAgeChange = (index, age) => {
    const newAges = [...formData.childrenAges];
    newAges[index] = parseInt(age, 10) || 0;
  ...
  const pricePreview = useMemo(() => {
    ...
    if (!from || !to || !formData.accommodationType) {
      return null;
    }
    ...
    const checkInStr = format(from, 'yyyy-MM-dd');
    const checkOutStr = format(to, 'yyyy-MM-dd');
    ...
    const standardTotalPrice = roundMoney(
      calculateTotalPrice(nightlyCosts.totalNightly, nights),
    );

    const nonRefundableDiscountAmount = roundMoney(
      standardTotalPrice *
        (NON_REFUNDABLE_DISCOUNT_PERCENT / 100),
    );

    const nonRefundableTotalPrice = roundMoney(
      standardTotalPrice - nonRefundableDiscountAmount,
    );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok || !availabilityData.available) {
        toast.error(
          `Dates unavailable: ${
    ...
    if (!pricePreview) {
      toast.error('Unable to calculate the booking price.');
      return;
    }

    const isNonRefundable =
      formData.rateType === 'non_refundable';

    const discountPercent = isNonRefundable
      ? NON_REFUNDABLE_DISCOUNT_PERCENT
      : 0;

    const discountAmount = isNonRefundable
      ? pricePreview.nonRefundableDiscountAmount
      : 0;

    const finalTotalPrice = isNonRefundable
      ? pricePreview.nonRefundableTotalPrice
    ...
    navigate('/booking/review', {
      state: {
        bookingData: {
          ...
          rateLabel: isNonRefundable
            ? 'Non-refundable'
            : 'Flexible',
          isRefundable: !isNonRefundable,

          cancellationPolicyCode: isNonRefundable
            ? 'non_refundable'
            : 'flexible_30_days',

          cancellationPolicyText: isNonRefundable
        ...
        priceData: {
          ...
          seasonMultiplier: pricePreview.seasonMultiplier,
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div>
              ...
              {selectedRoom &&
                selectedRoom.available_rooms > 0 &&
                selectedRoom.available_rooms < 3 && (
                  <div className="mt-3 p-3 bg-warning/10 rounded-md text-sm flex items-center border border-warning/20">
                    ...
                    Only {selectedRoom.available_rooms} room
                    {selectedRoom.available_rooms > 1 ? 's' : ''}{' '}
                    left!
                  </div>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    handleChildCountChange(event.target.value)
                  }
                  className="mt-2"
                />
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Select the cancellation conditions that suit
                  your plans.
                </p>
              </div>
                  ...
                  <div>
                    ...
                    <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                      <p className="flex gap-2">
                        ...
                        100% refund more than 30 days before
                        arrival.
                      </p>

                      <p className="flex gap-2">
                        <CheckCircle2 className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                        50% refund from 30 days until 2 days
                        before arrival.
                      </p>

                      <p className="flex gap-2">
                        <AlertCircle className="w-4 h-4 text-destructive shrink-0 mt-0.5" />
                        No refund within 2 days of arrival.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">
                      Save 10%. This rate cannot be refunded if
                    ...
                    {pricePreview && (
                      <p className="mt-3 text-sm font-medium text-emerald-700">
                        You save €
                        {pricePreview.nonRefundableDiscountAmount.toFixed(
                          2,
                        )}
                      </p>
                    )}
                  ...
                  <div className="text-right shrink-0">
                    {pricePreview && (
                      <div className="text-sm text-muted-foreground line-through">
                        €
                        {pricePreview.standardTotalPrice.toFixed(2)}
                      </div>
                    )}

                    <div className="text-xl font-bold text-primary">
                      {pricePreview
                        ? `€${pricePreview.nonRefundableTotalPrice.toFixed(
                            2,
                          )}`
                        : 'Select dates'}
                    </div>
